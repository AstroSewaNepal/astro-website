...
export async function GET(
  request: NextRequest,
  context: { params: { resource: string; slug: string } },
) {
  const { resource, slug } = context.params;

  if (!isGhostResource(resource)) {
    logger.warn('Unsupported Ghost resource requested by slug', {
      resource,
      slug,
    });
  ...
  try {
    ...
    logger.info('Fetched Ghost resource by slug', {
      resource,
      slug,
      options: readOptions,
    });
    ...
    if (isNotFoundError(error)) {
      logger.warn('Ghost resource not found by slug', { resource, slug });
      return NextResponse.json({ error: 'Resource not found' }, { status: 404 });
    }

    logger.error('Failed to fetch Ghost resource by slug', {
      resource,
      slug,
      options: readOptions,
